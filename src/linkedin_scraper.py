...
from src.config import LINKEDIN_PROFILE_URL


@dataclass
class LinkedInProfileData:
    """Structured representation of LinkedIn profile information."""
...
class LinkedInScraper:
    """Scrapes public LinkedIn profile with resilient fallback."""

    def __init__(self, profile_url: str = LINKEDIN_PROFILE_URL) -> None:
    ...
    def scrape_profile(
        ...
        """Attempt to scrape LinkedIn public profile, falling back to memory if blocked."""
        print(f"[LinkedInScraper] Attempting to scrape public profile: {self.profile_url}")
        ...
        try:
            ...
            if response.status_code in (401, 403, 999) or "login" in response.url.lower():
                print(
                    ...
                    "Using stored profile memory baseline."
                )
                return default_data
            ...
            # Try OpenGraph tags first (often intact even on public pages)
            ...
            if og_title and og_title.get("content"):
                ...
                # format usually "Name - Title | LinkedIn"
                if " - " in title_text:
            ...
            # Attempt to find headline or about in body tags
            h1 = soup.find("h1")
            ...
            # Recent activity/posts section if accessible
            activity_list: list[str] = []
            activity_section = soup.find("section", id=re.compile(r"activity|posts", re.I))
            if activity_section:
                for item in activity_section.find_all("li"):
                    text = item.get_text(strip=True)
                    if len(text) > 20:
                        activity_list.append(text[:300])

            print("[LinkedInScraper] Successfully extracted profile elements from page.")
            ...
            print(f"[LinkedInScraper] Scraping attempt error: {exc}. Proceeding with fallback.")
            return default_data
